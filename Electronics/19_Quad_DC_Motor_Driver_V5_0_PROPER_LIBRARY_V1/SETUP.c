...
void motorSetup()
{
	...
	M[0].channel = (uint32_t *) &TIM4->CCR3;
	M[1].channel = (uint32_t *) &TIM4->CCR4;
	M[2].channel = (uint32_t *) &TIM8->CCR4;
	M[3].channel = (uint32_t *) &TIM8->CCR3;
}
void E12BusSetup()
{
...
void contactSwitchSetup()
{
	for (uint8_t i = 0; i < 9; i++)
	{
		pinMode(GPIOE, i, INPUT);
		pinPull(GPIOE, i, PULLUP);
	}
	pinMode(GPIOC, 4, INPUT);
	pinMode(GPIOC, 5, INPUT);
	pinMode(GPIOC, 13, INPUT);
	pinPull(GPIOC, 4, PULLUP);
	pinPull(GPIOC, 5, PULLUP);
	pinPull(GPIOC, 13, PULLUP);
	pinMode(GPIOB, 0, INPUT);
	pinMode(GPIOB, 1, INPUT);
	pinMode(GPIOA, 4, INPUT);
	pinMode(GPIOA, 5, INPUT);
	pinPull(GPIOB, 0, PULLUP);
	pinPull(GPIOB, 1, PULLUP);
	pinPull(GPIOA, 4, PULLUP);
	pinPull(GPIOA, 5, PULLUP);
	
}
uint16_t readContactSwitch()
{
	uint8_t pinNumber[16] = { 
		0,
		1,
		2,
		3,
	...
	uint16_t contactSwitch = 0;
	for (uint8_t i = 0; i < 16; i++)
	{
		contactSwitch |= pinRead(port[i], pinNumber[i]) << i;
	}
